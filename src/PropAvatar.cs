...
namespace BonelabPropHunt
{
    ...
    public class PropAvatar : MonoBehaviour
    {
        public static void Switch(RigManager rigManager, GameObject prop)
        {
            ...
            if (!propAvatar)
            {
                propAvatar =
                    ...
                    ?? PropAvatarPrefab.Setup(
                        rigManager.CalibrationAvatar,
                        prop
                    );
        ...
        public static void SetBeltAndHolstersVisible(
        ...
        public static void SetRigCollisionExceptHeadHandsFeet(
            RigManager rigManager,
            bool isEnabled
        )
        {
            ...
            {
                transform.GetComponent<Rigidbody>().detectCollisions =
                    isEnabled;
            }
        }
        ...
        public void UpdateAvatarStats()
        {
            ...
            if (OnAfterUpdateAvatarStats != null)
                OnAfterUpdateAvatarStats(this);
        }

        // TODO: Remove these testing things/make logic permanent
        public static Action<PropAvatar, GameObject> OnBeforeUpdateAvatarStats =
            (propAvatar, prop) =>
            {
                foreach (var leg in new[] { "legLeft", "legRight" })
                {
                    var propBounds = Utils
                        .GetNestedComponents<Collider>(prop)
                        .Where(col => col.attachedRigidbody)
                        .Select(col => col.bounds)
                        .Aggregate(
                            (totalBounds, bounds) =>
                            {
                    ...
                    // TODO: Would center of gravity be better? Closest
                    //       RigidBody point to it?
                    var propPelvisAttachmentPoint = propBounds.center;
                    var polyblankLegHeight = 1f; // TODO
                    propAvatar.transform.Find(leg).localScale = new Vector3(
                        1f,
                        polyblankLegHeight / propPelvisAttachmentPoint.y,
                        1f
                    );
                }

                propAvatar.Avatar.PreComputed = false;
            };
        public static Action<PropAvatar> OnAfterUpdateAvatarStats;
    }
}
