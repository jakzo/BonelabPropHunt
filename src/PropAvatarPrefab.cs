...
public static class PropAvatarPrefab {
  ...
  private static SLZ.VRMK.Avatar _polyblankAvatar;

  public static GameObject Load(RigManager rigManager) {
    if (!_prefab) {
      var polyblank = rigManager.CalibrationAvatar.gameObject;
      InitPrefab(polyblank);
    }
    return _prefab;
  }

  // TODO: Do we still need this?
  public static void Load(Action<GameObject> onLoaded) {
    if (!_prefab) {
      var polyblankCrate = AssetWarehouse.Instance.GetCrate<AvatarCrate>(
          new Barcode("c3534c5a-94b2-40a4-912a-24a8506f6c79"));
      polyblankCrate.LoadAsset(new Action<GameObject>(asset => {
        InitPrefab(asset);
        onLoaded(_prefab);
      }));
    } else {
  ...
  private static void InitPrefab(GameObject polyblank) {
    _polyblankAvatar = polyblank.GetComponent<SLZ.VRMK.Avatar>();
    _prefab = GameObject.Instantiate(polyblank);
    _prefab.name = "prop_avatar";

    var propAvatar = _prefab.AddComponent<PropAvatar>();
    propAvatar.Avatar = _prefab.GetComponent<SLZ.VRMK.Avatar>();
    InitAvatar(propAvatar.Avatar);

    // TODO: Disabled for testing
    // HideLegsAndTorsoRings();

    _prefab.hideFlags = HideFlags.HideAndDontSave;
    _prefab.active = false;
  }

  private static void HideLegsAndTorsoRings() {
    var animator = _prefab.GetComponent<Animator>();
    foreach (var bone in new[] { HumanBodyBones.LeftUpperLeg,
                                 HumanBodyBones.RightUpperLeg }) {
      animator.GetBoneTransform(bone).localScale = Vector3.zero;
    }

    _prefab.transform.Find("SHJntGrp/MAINSHJnt/ROOTSHJnt/HipToSternumPointer")
        .localScale = Vector3.zero;
  }

  public static void InitAvatar(SLZ.VRMK.Avatar avatar,
                                SLZ.VRMK.Avatar polyblankAvatar = null) {
    if (!polyblankAvatar)
    ...
    avatar.requiredBonesOrRefsMissing =
        polyblankAvatar.requiredBonesOrRefsMissing;
    ...
    avatar._spineUnderbustToWaistPerc =
        polyblankAvatar._spineUnderbustToWaistPerc;
    avatar._spineWaistToHighHipsPerc =
        polyblankAvatar._spineWaistToHighHipsPerc;
    ...
    avatar._handSchematicLf_k__BackingField =
        polyblankAvatar._handSchematicLf_k__BackingField;
    avatar._l1OffsetZ = polyblankAvatar._l1OffsetZ;
    avatar._handSchematicRt_k__BackingField =
        polyblankAvatar._handSchematicRt_k__BackingField;
