...
public class Mod : MelonMod {
  ...
  public override void OnUpdate() {
    if (IsCtrlDown() && Input.GetKeyDown(KeyCode.P)) {
      ShapeShift(GameObject.Find("kitBash_medium_milkCrate_interactable [0]"));
    }

    if (_rigManager?.ControllerRig.rightController.GetThumbStick() ?? false) {
      if (!_isThumbstickDown) {
        _isThumbstickDown = true;
        var heldProp = new[] { _rigManager.physicsRig.leftHand,
  ...
  // [HarmonyPatch(typeof(SLZ.VRMK.Avatar),
  //               nameof(SLZ.VRMK.Avatar.ComputeBaseStats))]
  // class Avatar_ComputeBaseStats_Patch {
  //   [HarmonyPostfix()]
  //   internal static void Postfix(SLZ.VRMK.Avatar __instance) {
  //     if (!_isSwitchingToProp)
  //       return;
